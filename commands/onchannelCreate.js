                ...
                .then(function(snapshot) {
                    ...
                    if (log.child('logchannelid').val() != message.channel.guild.defaultChannel.id) {
                        if (log.child('ChannelCreate').val()) {
                            ...
                            eris.createMessage(message.channel.id, "Sistema de logs channelCreate foi desativado no server: " + message.channel.guild.name);
                        } else {
                            setlinkData.child('ChannelCreate').set(true);
                            eris.createMessage(message.channel.id, "Sistema de logs channelCreate foi ativado no server: " + message.channel.guild.name);
                        }
                    } else {
